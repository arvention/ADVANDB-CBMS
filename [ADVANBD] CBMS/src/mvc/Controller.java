...
public class Controller {
	...
	public String query7Builder() {
		...
		if (hasARCDP.equals("Nakatanggap")) {
			sql += "JOIN hpq_arcdp_mem A ON H.id = A.`main.id` AND A.arcdp_mem_refno = M.memno where H.prog_arcdp = 1";
		} else if (hasARCDP.equals("Hindi Nakatanggap")) {
			sql += "LEFT JOIN hpq_arcdp_mem A ON H.id = A.`main.id` AND A.arcdp_mem_refno = M.memno where A.`main.id` IS NULL";
		}
